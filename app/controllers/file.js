...
module.exports = {
  ...
  create: async (owner, name, description, raw) =>
  {
    ...
    else
    {
      ...
      //Write to disk
      await fs.writeFile(path.join(config.get('core.data.filesystem'), doc.id) + '.raw', raw);
  ...
  get: async _id =>
  {
    ...
    //Get from disk
    const raw = await fs.readFile(path.join(config.get('core.data.filesystem'), _id) + '.raw', 'utf8');
